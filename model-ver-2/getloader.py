...
class Vocabulary:
    """
        Класс словаря, описывающий структуры, содержащие слова, которые запоминает модель в процессе обучения и на
        основе которых формирует подпись к изображению.
    """

    def __init__(self, freq_threshold):
        """
            Функция инициализации, при котором генерируются объекты типа словарь (первый объект - определяет слово по
    ...
    def build_vocabulary(self, sentence_list):
        """
            Функция осуществляет заполнение словаря (добавление слов в него и определение для каждого из них индекса).
...
class FlickrDataset(Dataset):
    ...
    def __init__(self, root_dir, caption_file, transform=None, freq_threshold=5):
        """
            Инициализирует объект датасета, определяя списки путей к изображениям и соответствующим им подписям, а также
            формирует словарь.
    ...
    def __getitem__(self, index):
        """
            Функция определяет возможность взятия одной сущности из датасета (в частности, изображения и соответствующей
            ему подписи).

        :param index: индекс сущности, которую необходимо считать
        :return: кортеж из трансформированного изображения и тензора преобразованной в индексы слов подписи
        """
...
def get_loader(
    ...
    """
    Функция get_loader осуществляет загрузку данных для работы модели машинного обучения с этими данными.
